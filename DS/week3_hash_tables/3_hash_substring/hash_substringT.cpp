...
unsigned long long poly_hash( const string& s ) {
    ...
    for ( int i = static_cast< int >( s.size() ) - 1; i >= 0; --i ) hash 
= ( hash * multiplier + s[i] ) % prime;
    return hash;
}

std::vector< unsigned long long > precompute_hashes( const std::string& 
T, const std::string& P, long long p,
        long long x ) {
    ...
    for ( int i = static_cast< int >( T.size() - P.size() - 1 ); i >= 0; 
--i ) {
