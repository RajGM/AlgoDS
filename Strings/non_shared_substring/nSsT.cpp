...
struct SuffixTree {
    ...
    NodeProperties search_leafs(int node, string s = "")
    {
        ...
        for (Node& n : tree[node]) {
            NodeProperties np = search_leafs(n.next, s + 
text.substr(n.start, n.end - n.start));

            if (np == NodeProperties::OnlyLTypeLeafs) {
                is_L_type_leaf = true;
                if (node == 0) {
                    substrings.emplace_back(text.substr(n.start, n.end - 
n.start));
                }
            }
        ...
        for (int j = 0; j < LENGTH; ++j) {
            ...
            for (int k = j; true;) {
                ...
                if (m == node.end && k == LENGTH) {
                    ...
                    tree.emplace_back(Edges{});
                    tree[cnt].emplace_back(Node(node.next, m, 
node.end));
                    node.next = cnt;
                    node.end = m;
                    break;
