...
std::vector<int> sortDoubled(const std::string& s, int L, const 
std::vector<int>& order, const std::vector<int>& cclass)
{
...
std::vector<int> computeCharacterClasses(const std::string& s, const 
...
std::vector<int> updateClasses(const std::vector<int>& newOrder, const 
std::vector<int>& cclass, int L)
{
    ...
    for (int i = 1; i < n; ++i) {
        ...
        if (cclass[cur] != cclass[prev] || cclass[mid] != 
cclass[midPrev]) {
